...
class AiMonitoringChatSubscriber extends AiMonitoringSubscriber {
  ...
  recordChatCompletionEvents({ ctx, request, response, err, timeOfFirstToken, metadata = {}, tags = [] }) {
    ...
    if (err) {
      const llmError = this.createErrorMessage({ cause: err, summary: completionSummary, response })
      this.agent.errors.add(ctx.transaction, err, llmError)
    }
  }

  /**
   * Creates an LLM error message instance. Override in vendor-specific
   * subscribers to return vendor-specific error message subclasses.
   *
   * @param {object} params Constructor parameters for LlmErrorMessage
   * @returns {LlmErrorMessage} an LLM error message instance
   */
